...
float shadowStrength(vec3 fragpos)
{
	vec4 shadowClip	= globalUBO.light.transform * vec4(fragpos, 1.0);

	float shadowFactor;

	return textureProj(shadowClip, vec2(0.0));
}
...
void main()
{
    ...
    const float visibility = clamp(shadowStrength(world_pos), Diffuse, 1.0f);
    const float ao         = 1.0f;
